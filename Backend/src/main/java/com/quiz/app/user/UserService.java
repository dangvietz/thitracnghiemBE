...
public class UserService {
    ...
    public void storeResetPasswordCode(String email, String resetPasswordCode, LocalDateTime expirationTime) {
        // Implement the logic to store the reset password code and expiration time in the database.
        // You should use a database or some data storage mechanism for this purpose.
        // This can vary depending on the database technology you are using.

        // For example, if you're using Spring Data JPA and have a User entity:
        User user = userRepository.findByEmail(email);
        if (user != null) {
            user.setResetPasswordCode(Integer.valueOf(resetPasswordCode));
            user.setResetPasswordExpirationTime(expirationTime);
            userRepository.save(user); // Assuming you are using Spring Data JPA repository.
        }
    }
    public String generateUniqueCode() {
        // Generate a unique code (you can customize the length and format)
        int codeLength = 6; // Set the desired code length
